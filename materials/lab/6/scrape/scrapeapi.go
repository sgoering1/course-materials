    ...
    return func(path string, f os.FileInfo, err error) error {
        ...
        for j, r := range regexes {
            if r.MatchString(path) {
				...
				for i, match := range(Files){
					if match.Filename == tfile.Filename && match.Location == tfile.location{
						return nil
					}
				}
                ...
                if w != nil && len(Files)>0 {
                    ...
                    w.Write([]byte(`"`+(strconv.FormatInt(int64(j))+`":  `)))
...
func walkFn2(w http.ResponseWriter, query string) filepath.WalkFunc {
    return func(path string, f os.FileInfo, err error) error {
		single_reg := regexp.MustCompile(query)
		if single_reg.MatchString(path){
			var tfile FileInfo
            dir, filename := filepath.Split(path)
            tfile.Filename = string(filename)
            tfile.Location = string(dir)
...
func MainPage(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Fprintf(w, "<html><body><H1>This pages scrapes files for a match using regex regular expressions</H1></body>")
}
